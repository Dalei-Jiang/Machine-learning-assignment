...
#   AStar_single    (15) Tiny, Small, Open          (10) Medium, Large
#   AStar_multiple  (20) Tiny, Open, One_d, Tiny_c  (25) Cross, Small, Medium, Medium_c, Large,
#   Suboptimal      (5) Medium                      (5) Large
# ------------------------------
...
def main():  
    ...
    mazes = (
        ...
        {case: maze.Maze('data/part-3/{0}'.format(case))
            for case in ('tiny', 'open', 'tiny_c', 'one_d')}, # 'cross', 'small', 'medium',
        # extra credit-part 4: 10 points total, 5 points per case
    ...
    first_parts    = tuple(item for i, points in zip(range(0, 3), (1, 1, 1))
        for item in grade_optimal('part-{0}'.format(i + 1), key[i], mazes[i], solutions[i], 
            weight = points))
    last_part      = tuple(item for i in range(3, 4) for item in grade_suboptimal('extra credit-part-{0}'.format(i + 1), key[i], mazes[i], solutions[i]))
...
if __name__ == "__main__":
    ...
    end_time = time.time()
    print("The running time: ", end_time - begin_time)
