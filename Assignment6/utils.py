...
def rollout(env: gym.Env, policies: policies.QPolicy, episodes: int, epsilon: float, render: bool = False):
    ...
    for _ in range(episodes):
        ...
        while not done:
            ...
            # print("The pi is:", pi)
            # print("The length of the pi is:", len(pi))
            # input()
            # How do you select the action given pi. Hint: use np.random.choice
            action = np.random.choice(range(len(pi)),p=pi) # 从pi中按照概率epsilon随机选择一个步骤
            ### Your code ends here.
            next_state, reward, done, _ = env.step(action) # 环境执行了行动，返回了下一个状态，和reward，以及是否结束。
            score += reward
            replay.append((state, action, reward, next_state, done)) # 根据定义，replay包含了每一步的(state, action, reward, next_state, done)
            state = next_state
            step += 1
        env.close() # 关闭环境
        scores.append(score)
    return replay, scores # 返回的值为所有的模拟元组结果，和每一个模拟的最后分数
