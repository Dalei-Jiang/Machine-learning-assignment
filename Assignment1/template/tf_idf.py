...
def compute_tf_idf(train_set, train_labels, dev_set):
    ...
    for mail in dev_set:
        tf_idf_dict = {}
        Num_words_mail = len(mail)
        vocabulary = dict(Counter(mail))
        key_list = vocabulary.keys()
        for word in key_list:
            Num_docs_contain = 0
            for docs in train_set:
                if word in docs:
                    Num_docs_contain += 1
            tf_idf = vocabulary[word]/Num_words_mail*math.log(Num_doc/(1+Num_docs_contain))
            tf_idf_dict[word] = tf_idf
        max_key = max(tf_idf_dict, key = tf_idf_dict.get)
        tf_list.append(max_key)
    # return list of words (should return a list, not numpy array or similar)
    return tf_list